...
export const PasswordRule = (values, value) => {
  ...
  if (!onlySpecialChars.length) return 'A senha precisa conter pelo menos 1 caracter especial.'

  // Get only uppercase chars
  const onlyUpperCaseChars = value.replace(/[^A-Z]/g, '')
  if (!onlyUpperCaseChars.length) return 'A senha precisa conter pelo menos 1 letra maiúscula.'
